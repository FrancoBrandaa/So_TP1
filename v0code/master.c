...
void cleanup_resources(void) {
    ...
    if (player_pipes) {
        for (int i = 0; i < player_count; i++) {
            if (player_pipes[i]) {
                if (player_pipes[i][0] != -1) close(player_pipes[i][0]);
                if (player_pipes[i][1] != -1) close(player_pipes[i][1]);
                free(player_pipes[i]);
            }
        }
        free(player_pipes);
    ...
    if (game_state) munmap(game_state, sizeof(game_state_t) + sizeof(int) * game_state->width * game_state->height);
    if (game_sync) munmap(game_sync, sizeof(game_sync_t));
    if (state_shm_fd != -1) close(state_shm_fd);
    if (sync_shm_fd != -1) close(sync_shm_fd);
    
    shm_unlink(GAME_STATE_SHM);
    shm_unlink(GAME_SYNC_SHM);

    if (player_pids) free(player_pids);
}
...
void place_players(game_config_t *config) {
    ...
    int positions[][2] = {
        {0, 0}, {config->width-1, 0}, {0, config->height-1}, {config->width-1, config->height-1},
        {config->width/2, 0}, {config->width/2, config->height-1},
        {0, config->height/2}, {config->width-1, config->height/2},
...
bool process_move(int player_id, unsigned char direction) {
    if (player_id < 0 || (unsigned int)player_id >= game_state->player_count) return false;
